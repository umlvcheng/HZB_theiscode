...
for n=1:N
  if ~isnan(goodpts(n))
    dist=sqrt((mesh.uvnode(:,1)-mesh.uvnode(n,1)).^2+(mesh.uvnode(:,2)-mesh.uvnode(n,2)).^2);
...
plot(mesh.uvnode(:,1)/1e3,mesh.uvnode(:,2)/1e3,'r.',mesh.uvnode(goodpts,1)/1e3,mesh.uvnode(goodpts,2)/1e3,'b.');  
